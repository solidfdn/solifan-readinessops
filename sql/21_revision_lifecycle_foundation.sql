...
USING (
    SELECT
        CURRENT_DATABASE() AS ENVIRONMENT_NAME,
        IFF(CURRENT_DATABASE() = 'READINESSOPS_VALIDATION', 'PRODUCTION', 'DEV') AS ENVIRONMENT_TYPE,
        IFF(CURRENT_DATABASE() = 'READINESSOPS_VALIDATION', FALSE, TRUE) AS ALLOW_DEMO_RESET,
        FALSE AS ALLOW_HARD_PURGE
) source
ON target.ENVIRONMENT_NAME = source.ENVIRONMENT_NAME
WHEN MATCHED THEN UPDATE SET
    ENVIRONMENT_TYPE = source.ENVIRONMENT_TYPE,
    ALLOW_DEMO_RESET = source.ALLOW_DEMO_RESET,
    ALLOW_HARD_PURGE = IFF(source.ENVIRONMENT_TYPE = 'PRODUCTION', FALSE, target.ALLOW_HARD_PURGE),
...
DELETE FROM READINESSOPS_ENVIRONMENT_CONTROL
